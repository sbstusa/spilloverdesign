...
chooseBestPairs<-function(pm,numpairs){
  ...
  if(length(pm) < numpairs){ numpairs<-length(pm) } ## can't choose more than total number of pairs
  bestpairs<-row.names(tmp)[1:numpairs]
  return(bestpairs)
}

numpairs<-5
selectedPairs<-lapply(pairsByState,function(pm){ chooseBestPairs(pm,numpairs) })
