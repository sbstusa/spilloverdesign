...
## change encoding of some non-English characters like Doña Ana County, NM, doing it by hand in vim:
### s/Do?a/Dona/gc
```
...
## Distance between counties in terms of number of farmers
numfarmers<-wrkdat$n
names(numfarmers)<-row.names(wrkdat)
sizeDist<-scalarDist(numfarmers)

## Distance between centroids of population
popCentroidDist<-as.matrix(dist(wrkdat[,c("LATITUDE","LONGITUDE")]))

## Setup the penalty
sizeDist[1:5,1:5]
popCentroidDist[1:5,1:5]

maxDists<-apply(popCentroidDist,1,max)

tmp<-popCentroidDist
diag(tmp)<-0

sizeDist[1:5,1:5]+tmp[1:5,1:5]

thedist<-1000*sizeDist+tmp

thedistDM<-distancematrix(thedist)
...
pairsByState<-lapply(split(wrkdat,wrkdat$state), function(dat){
		       ...
		       sizeDist<-distancematrix(scalarDist(numfarmers))
		       nbpm<-nonbimatch(sizeDist)
