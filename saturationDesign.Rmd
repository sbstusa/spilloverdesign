...
## Choose best pair within each state
...
chooseBestPair<-function(pm){
  ...
  thebestpair<-bestpairs[avgN[names(bestpairs)]==max(avgN[names(bestpairs)])] ## among closest matches, choose max N
  return(names(thebestpair))
}


set.seed(20160101)
sampledPair<-lapply(pairsByState,function(pm){ message(pm[1]); chooseBestPair(pm) })
