...
chooseBestPairs<-function(pm,numpairs){
  ...
  ## tmp<-cbind(sizeDiffs,avgN)[order(sizeDiffs,-1*avgN,decreasing=FALSE),] ## sort by diff in size and then by avgN
  tmp<-cbind(sizeDiffs,avgN,sizeDiffs/avgN)[order(sizeDiffs/avgN,-1*avgN,decreasing=FALSE),] ## sort by diff in size/avgN and then by avgN
  ...
  #####loop starts
  pair1<-strsplit(row.names(newtmp)[1],"-")[[1]]
  names(pair1)<-rep(row.names(newtmp)[1],2)
  pair1Z<-sample(c(0,1)) ## assign one county to treatment
  nFarmers1<-pairedDat[pair1[pair1Z==1],"n"]/2
  nFarmers<-nFarmers1+nFarmers
  remainingPairs<-row.names(newtmp)[!(row.names(newtmp) %in% names(pair1)[1])]
  adjM<-adjMat[pair1[pair1Z==0],
	       unlist(strsplit(remainingPairs,"-"))]
  adjCounties<-names(adjM)[adjM==1]
  adjCountiesLocs<-sapply(c(pair1[1],adjCounties),function(cnty){ grep(cnty,row.names(tmp)) })
  includeCounties<-row.names(newtmp)[-c(adjCountiesLocs)]
  newtmp<-newtmp[includeCounties,]
  


  nAvail <- max(nrows(tmp[-1,]),numpairs)
  chosenPairs<-vector(length=numpairs)
  chosenPairs[1,]<-pair1
  while(nAvail>0){
    ## choose next pair keeping the control county in the first pair isolated (within state only, this doesn't control cross state adjacency
  adjM<-adjMat[pair1[pair1Z==0],
	       unlist(strsplit(row.names(tmp)[!(row.names(tmp) %in% chosenPairs),"-"))]
  adjCounties<-names(adjM)[adjM==1]

  badpairs<-sapply(adjCounties,function(cnty){ grep(cnty,row.names(tmp)) })
  tmp2<-tmp[-c(1,badpairs),]
  pair2<-strsplit(row.names(tmp2)[1],"-")[[1]]
  pair2Z<-sample(c(0,1))
  n2<-pairedDat[pair2[pair2Z==1],"n"]/2
  nAvail<-nAvail-1
  }


  pair1s<-row.names(tmp)[1:numpairs]
  return(pair1s)
}

##maxnumpairs<-5
selectedPairs<-lapply(names(pairsByState),function(nm){
			nm<-names(pairsByState)[[1]]
			numpairs<- floor(length(pairsByState[[nm]])/2) ## no more than 1/4 of the counties in a state in treatment
			chooseBestPairs(pairsByState[[nm]],numpairs) 
							   })

## Turn the list into a data frame
tmp<-lapply(names(selectedPairs),function(x){
	      ## message(x)
	      if(length(selectedPairs[[x]])<numpairs){ numpairs<-length(selectedPairs[[x]]) }
	      cbind(state=rep(x,2*numpairs),
		    pair=rep(selectedPairs[[x]],each=2),
		    county=unlist(strsplit(selectedPairs[[x]],"-")))
							   })
experimentDat<-data.frame(do.call("rbind",tmp),stringsAsFactors = FALSE)
row.names(experimentDat)<-experimentDat$county
experimentDat$n<-pairedDat[row.names(experimentDat),"n"]
experimentDat[1:10,]
```
...
```{r}
set.seed(20150101)
experimentDat$Z<-unsplit(lapply(split(experimentDat$county,experimentDat$pair),function(x){ sample(c(0,1)) }),experimentDat$pair)
experimentDat[1:10,]

stopifnot(all(tapply(experimentDat$Z,experimentDat$pair,sum)==1)) ## exactly 1 treated county per pair

```

An attempt at a map of our counties. Black are those counties assigned treatment. I think that this is very useful since we may want to go back and refine the algorithm a bit.

```{r map, fig.show='asis',fig.keep='high'}
png(file="map.png",width=1280,height=800)
par(oma=rep(0,4))
library(maps)
data(countyMapEnv)
data(usaMapEnv)
