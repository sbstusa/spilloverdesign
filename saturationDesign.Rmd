...
mnList<-lapply(mnfips,function(x){
		  mnfips %in% adjacentCountiesFIPS[[x]]
	       })
names(mnList)<-mnfips
...
getStatePairs<-function(fips){
  ...
  penMat<- distancematrix( sizeD + adjD*maxSizeDist )  ## add the maximum distance to any entry where two counties are adjacent
  nbpm<-nonbimatch(penMat)
...
pairsByState<-lapply(1:length(fipsByState),function(i){
		       message(names(fipsByState)[i])
		       fips<-fipsByState[[i]]
		       getStatePairs(fips)})
