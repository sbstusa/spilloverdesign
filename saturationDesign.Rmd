...
We want roughly 3 pairs within each state.
...
chooseBestPair<-function(pm){
  ## Given a vector of pair match labels, choose the best pair (closest match in size that is not adjacent with largest sample)
  okpairs<-names(pairDiffsN[pm][pairDiffsAdj[pm]]) ## exclude adjacent pairs
  sizeDiffs<-pairDiffsN[okpairs]
  avgN<-pairAvgN[okpairs]
  bestpairs<-sizeDiffs[sizeDiffs==min(sizeDiffs)] ## select closest matched pairs
  thebestpair<-bestpairs[avgN[names(bestpairs)]==max(avgN[names(bestpairs)])] ## among closest matches, choose the one with max N
  return(names(thebestpair))
}


set.seed(20160101)
selectedpair<-lapply(pairsByState,function(pm){ chooseBestPair(pm) })
stopifnot(all(sapply(selectedpair,length)==1))

tmp<-lapply(names(selectedpair),function(x){
	      cbind(state=rep(x,2),county=strsplit(selectedpair[[x]],"-")[[1]])
							   })
...
```{r}
set.seed(20150101)
experimentDat$Z<-unsplit(lapply(split(experimentDat$county,experimentDat$state),function(x){ sample(c(0,1)) }),experimentDat$state)

stopifnot(all(tapply(experimentDat$Z,experimentDat$state,sum)==1))

```

An attempt at a map of our counties. Black are those counties assigned treatment.
```{r}
library(maps)
data(countyMapEnv)
data(usaMapEnv)
data(county.fips)
map('state',regions=states$State[states$Postal.Code %in% experimentDat$state],exact=FALSE)
map("county",regions=county.fips$polyname[county.fips$fips %in% experimentDat$county],add=TRUE)
map("county",regions=county.fips$polyname[county.fips$fips %in% experimentDat$county[experimentDat$Z==1]],fill=TRUE,add=TRUE)
```
...
sum(experimentDat$n[experimentDat$Z==1])
